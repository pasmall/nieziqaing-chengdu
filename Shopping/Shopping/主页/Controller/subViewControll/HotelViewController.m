...
@interface HotelViewController()<UITableViewDataSource , UITableViewDelegate , SelectedConditionDelegate>{
    UIButton *countBtn;
}
...
- (void)loadMoreDeals{
    ...
    NSString *httpArg = [NSString stringWithFormat:@"city_id=%@&cat_ids=377&sort=%d&page=%d&page_size=10" , _cityId ,_conditonView.index,count ];
...
- (void)getBaiduDealsWithCityId:(NSNumber *)cityId{
    ...
    NSString *httpUrl = @"http://apis.baidu.com/baidunuomi/openapi/searchdeals";
    NSString *httpArg = [NSString stringWithFormat:@"city_id=%@&cat_ids=377&page_size=10" , cityId];
    NSString *urlStr = [[NSString alloc]initWithFormat: @"%@?%@", httpUrl, httpArg];
