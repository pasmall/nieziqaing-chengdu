...
-(void)initViews{
    //
    _topicImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, MainW, 160)];
    _topicImage.image = [UIImage imageNamed:@"bg_customReview_image_default"];
    [self.contentView addSubview:_topicImage];
    
    //背景
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, 160, MainW, 40)];
    [self.contentView addSubview:backView];
    
    //组
    for (int i = 0; i < 4; i++) {
        UIButton *segBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        segBtn.frame = CGRectMake(i*MainW/4, 0, MainW/4, 40);
        segBtn.tag = 30+i;
    ...
    //下划线
    _lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 38, MainW/4, 2)];
    _lineView.backgroundColor =  RGB(33, 192, 174);
...
-(void)setTopic:(DisTopicModel *)topic labelsArr:(NSMutableArray *)labels{
    ...
    for (int i = 0; i < labels.count; ++i) {
        ...
        UIButton *btn = (UIButton *)[self.contentView viewWithTag:30+i];
        btn.hidden = NO;
        btn.frame = CGRectMake(i*MainW/lcount, 0, MainW/lcount, 40);
